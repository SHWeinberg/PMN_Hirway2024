...
% addAttachedFiles(gcp,Emat);
% addAttachedFiles(gcp,Emat);
% addAttachedFiles(gcp,gridmat);

%% Start Run
IDval=2000000;
IDrange=[IDval:1:IDval+24];
IDrange=reshape(IDrange,5,5);
rngrange=[16221:1:16225];
rngcolumn=0;
for popdiff=[0.5]
    ...
    %popdiff=[0:0.25:1]
    %popdiff=0.5;
    beforetime=100;  % before MCS duration
    ...
    tumor=stateV2(1:tumorcells,:); % Create Phenotype for Tumor Cells
    ...
    load(Emat) % Load Less Transformed Phenotype
    ...
    pop_indexafter = pop_index{1,end};
    ...
    othervalues{i+11,1}='tumorcells';
    othervalues{i+11,2}=tumorcells;
