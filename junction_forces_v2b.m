function [jx,jy,jxnet,jynet,fxnet,fynet] = junction_forces_v2b(ctag, atag, NRc,...
...
for a=1:NRc
    ...
    lim=length(cid); allcluster = [];
    cellnodes_cell = cell(1,lim);
    cellnodes_count = cell(1,lim);
    for i=1:lim
        ...
        cellnodes_cell{i} = cellnodes;
        allcluster = [allcluster cellnodes];
    end
    for i = 1:lim
        cellnodes = cellnodes_cell{i};
        cellnodes_count_i = zeros(1,length(cellnodes));
        for q = 1:length(cellnodes)
            cellnodes_count_i(q) = sum(allcluster==cellnodes(q));
        end
        cellnodes_count{i} = cellnodes_count_i;
    end
    for i = 1:lim
       c=cid(i);
       cellnodes = cellnodes_cell{i};
       cellnodes_count_i = cellnodes_count{i};
       NRcelln = length(cellnodes);
        ...
        for p=1:NRcelln
            ...
            for q=1:NRjxnn
                ...
                dny=(n2y-ny)/cellnodes_count_i(p); % y distance between n and n2, normalizing by the number of pixels sharing that node
                dnx=(n2x-nx)/cellnodes_count_i(p); % x distance between n and n2
                fxnet(c)=fxnet(c)+dnx;
                fynet(c)=fynet(c)+dny;
            end
        end
