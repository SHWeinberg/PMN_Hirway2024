...
% Start Run
IDval=1000122;
IDrange=[IDval:1:IDval+49];
IDrange=reshape(IDrange,5,10);
rngrange=[111221:1:111225];
rngcolumn=0;
for tumorcells=1
    %popdiff=[0:0.25:1]
popdiff=[0.5];
rngcolumn=tumorcells;
...
for repeated=1
    ...
    IDval=1000171+1; %IDrange(repeated+3,rngcolumn)
    rngvalue=rngrange(repeated+3);
    %rngvalue=rngvalue+1;
...
%     matrvars=reshape(matrvars,3*dimval*dimval,1);
   ...
   % i=3;
    ...
    tgfbval=5; %tgfrange(i); % ExoTGFB added to the system
   ...
   % subplot(1,2,1)
    ...
    mcsval=10; %********************** MCS for SECOND PART OF SIMULATION
    aftermcsval=mcsval;
    clc
