...
% Switch GF sensitivity- so Stellate Cells have same sensisitivity as
% Kupffer cells

poolsize1=10;
%pool = parpool(poolsize1);
IDval=1000954;
IDrange=[IDval:1:IDval+9];
IDrange=reshape(IDrange,poolsize1,1);
...
%     matrvars=reshape(matrvars,3*dimval*dimval,1);
    ...
    for i1=1:size(pop_index,1)
        ...
        if(pop_index(i1)==1) % Kupffer
            ...
            tempparam(i1,paramvalue)= tempparam(i1,paramvalue)*(1+diff1/10); % Decreased TGFB sensitivity
          % KUPFFER CELLS HAVE SLIGHTLY LESS GF SENSITIVITY
        ...
        elseif(pop_index(i1)==2) %Stellate
          ...
          %  tempparam(i1,paramvalue)= tempparam(i1,paramvalue);
             tempparam(i1,paramvalue)= tempparam(i1,paramvalue)*(1+diff1/10); % Decreased TGFB sensitivity
             % STELLATE CELLS HAVE SLIGHTLY LESS GF SENSITIVITY
            % celltype='S';
