...
% Shreyas Hirway
% 3/4/21
% Do binning and values over time

%% Binning **************************************************************

tic
load('test_100p_305mcs_5tgfb_0.25tmax_0.06spread_targetscale2_inelasticity_1000_Jhalf10212.6743_dividing1.25_extra_A_BT0.963_ZEBscale0.8_R200scale1.2_.mat') %_inelasticity_1000
endt=size(ctag,2);
dim=sqrt(size(ctag,1));
bin=10; % Specify Bin Size
tp=endt; %timepoint
...
a=statevars{1,endv};
b=a(00001:10000);
c=reshape(a(00001:10000),100,100);

solT1=reshape(a(00001:10000),100,100);
solE1=reshape(a(10001:20000),100,100);
asmE1=reshape(a(20001:30000),100,100);
ecmT1=reshape(a(30001:40000),100,100);
...
binsize=dim/bin;
solTvec=zeros(1,bin^2);
solEvec=zeros(1,bin^2);
asmEvec=zeros(1,bin^2);
ecmTvec=zeros(1,bin^2);

count=1;
for x=1:bin
    for y=1:bin
        %disp([num2str((20*(x-1)+1)),':', num2str((20*(x))),',',num2str((20*(y-1)+1)),':',num2str((20*(y)))]);
        val1=mean(mean(solT1((binsize*(y-1)+1):(binsize*(y)),(binsize*(x-1)+1):(binsize*(x))), 'omitnan'),'omitnan');
        disp([num2str(y), ',' num2str(x), ': ', num2str(val1)]);
        solT2((binsize*(y-1)+1):(binsize*(y)),(binsize*(x-1)+1):(binsize*(x)))=val1;
        solTvec(count)=val1;
        
        val2=mean(mean(solE1((binsize*(y-1)+1):(binsize*(y)),(binsize*(x-1)+1):(binsize*(x))), 'omitnan'),'omitnan');
        solE2((binsize*(y-1)+1):(binsize*(y)),(binsize*(x-1)+1):(binsize*(x)))=val2;
        solEvec(count)=val2;
        
        val3=mean(mean(asmE1((binsize*(y-1)+1):(binsize*(y)),(binsize*(x-1)+1):(binsize*(x))), 'omitnan'),'omitnan');
        asmE2((binsize*(y-1)+1):(binsize*(y)),(binsize*(x-1)+1):(binsize*(x)))=val3;
        asmEvec(count)=val3;
        
        val4=mean(mean(ecmT1((binsize*(y-1)+1):(binsize*(y)),(binsize*(x-1)+1):(binsize*(x))), 'omitnan'),'omitnan');
        ecmT2((binsize*(y-1)+1):(binsize*(y)),(binsize*(x-1)+1):(binsize*(x)))=val4;
        ecmTvec(count)=val4;
...
subplot(2,2,1)
imagesc(solT2);
title(['solT at MCS:' num2str(endv)]);
colorbar
%caxis([0 max(maxsolT)]);
