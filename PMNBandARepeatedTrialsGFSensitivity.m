...
% No parallel computing- TGFB value
% Switch GF sensitivity- so Stellate Cells have less sensisitivity and
% Kupffer cells have regular sensitivity

poolsize1=10;
%pool = parpool(poolsize1);
IDval=1000924;
IDrange=[IDval:1:IDval+29];
IDrange=reshape(IDrange,poolsize1,3);
rngrange=[128221:1:128230];%[128221:1:128226];
rngcolumn=0;
a=cell(poolsize1,3);

for tgfbindex=16:1:18
    tumorindex=5;
    ...
    TGFBparameterrange=[0:0.5:7,10,15,20];
    beforeTGFB=TGFBparameterrange(tgfbindex);
...
rngcolumn=tgfbindex-15; % -11 to account for ID range only being 4 columns but tgfb range is 15 vars
...
%     matrvars=reshape(matrvars,3*dimval*dimval,1);
    ...
    for i1=1:size(pop_index,1)
        ...
        if(pop_index(i1)==1) % Kupffer
          ...
          % KUPFFER CELLS HAVE NORMAL GF SENSITIVITY
        ...
        elseif(pop_index(i1)==2) %Stellate
          ...
          %  tempparam(i1,paramvalue)= tempparam(i1,paramvalue);
             tempparam(i1,paramvalue)= tempparam(i1,paramvalue)*(1+diff1); % Decreased TGFB sensitivity
             % STELLATE CELLS HAVE LESS GF SENSITIVITY
            % celltype='S';
